...
) : IPostContext
{
    ...
    public async Task<IEnumerable<Post>> ReadAsync()
    {
        ...
        string[] directories = Directory.GetDirectories(_options.MarkdownPath);
        ...
        foreach (string directory in directories)
        {
            ...
            string category = directory.Split(Path.DirectorySeparatorChar).Last();
    ...
    private async Task<Post?> ReadAsync(string markdownPath, string category)
    {
        ...
        var datetime = File.GetLastWriteTime(markdownPath);
        ...
        string? header = null;
        var headerMatch = HeaderImgRegex().Match(content);
